...
class Minimax:
    ...
    def find(self, state: State, n_player: int, thinking_time: float) -> Tuple[str, str]:
        ...
        best_movement = self.minimax2(state2, n_player, 7, -9999999, 9999999, -1, -1, True)
        # print(best_movement)
    ...
    def minimax(self, state: State, player: int, depth: int, alpha: int, beta: int, col: int, row: int, maximizing: bool) -> Tuple[int, str, int, int]:
        ...
        if depth == 0 or win_c != None or self.thinking_time - time() < 0.1:
            ...
            # if maximizing:
            #     al = evaluate(state.board, player)
            # else:
            #     al = evaluate(state.board, 1 - player) * -1
        ...
        if maximizing == True:
            ...
            for col_ in range(state.board.col * 2):
                ...
                val = self.minimax(copy.deepcopy(state), player, depth - 1, alpha, beta, col__, check, False)
                if val[0] > max_val:
            ...
            for col_ in range(state.board.col * 2):
                ...
                col__ = col_ // 2
                check = place(state, 1 - player, piece, col__)
    ...
    def minimax2(self, state: State, player: int, depth: int, alpha: int, beta: int, col: int, row: int, maximizing: bool) -> Tuple[int, str, int, int]:
        pos_infinite = 9999999
        neg_infinite = -9999999
        win_c = is_win(copy.deepcopy(state.board))
        if depth == 0 or win_c != None or self.thinking_time - time() < 0.1:
            ...
            # else: al = evaluate(state.board, 1 - player)

            al = evaluate(state.board, player)
            if not maximizing: al = evaluate(state.board, 1 - player) * -1
        ...
        if maximizing:
            ...
            for col_ in range(state.board.col * 2):
                ...
                if col_ % 2 == 0:
                    ...
                    val = self.minimax2(board2, player, depth - 1, alpha, beta, col__, check, False)
                    if val[0] > max_val:
                        max_val, max_val_piece, max_val_row, max_val_col = val
                    alpha = max(alpha, val[0])
                    if val[0] >= beta: break
            ...
            for col_ in range(state.board.col * 2):
                ...
                val = self.minimax2(board2, player, depth - 1, alpha, beta, col__, check, True)
                if val[0] < min_val:
                    min_val, min_val_piece, min_val_row, min_val_col = val
                beta = min(beta, val[0])
                if val[0] <= alpha: break
